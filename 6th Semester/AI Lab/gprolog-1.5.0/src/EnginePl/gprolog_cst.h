
#define __GNU_PROLOG__        	1
#define __GPROLOG__           	1
#define __GPROLOG_MINOR__     	5
#define __GPROLOG_PATCHLEVEL__	0
#define __GPROLOG_VERSION__   	10500

#define PROLOG_DIALECT  	"gprolog"
#define PROLOG_NAME     	"GNU Prolog"
#define PROLOG_VERSION  	"1.5.0"
#define PROLOG_DATE     	"Jul  8 2021"
#define PROLOG_COPYRIGHT	"Copyright (C) 1999-2021 Daniel Diaz"

#define PL_MIN_INTEGER		-1152921504606846976
#define PL_MAX_INTEGER		1152921504606846975

#define PL_REF        		0 
#define PL_LST        		1 
#define PL_STC        		2 
#define PL_ATM        		3 
#define PL_FLT        		4 
#define PL_FDV        		5 
#define PL_INT        		7 
