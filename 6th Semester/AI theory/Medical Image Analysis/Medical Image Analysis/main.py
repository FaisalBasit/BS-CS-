...
import json
import asyncio
from agent_helper import get_treatment_plan  # Import agent logic
import markdown 
...
def index():
    if request.method == 'POST':
        ...
        if file and detection_type:
            ...
            # Generate treatment plan using async agent
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            treatment_plan = loop.run_until_complete(get_treatment_plan(result))

             # Ensure this is at the top of your file
            markdown_plan = markdown.markdown(treatment_plan)


            return render_template('index.html',
                                   result=result,
                                   confidence=f"{confidence * 100:.2f}%",
                                   treatment_plan=markdown_plan,
