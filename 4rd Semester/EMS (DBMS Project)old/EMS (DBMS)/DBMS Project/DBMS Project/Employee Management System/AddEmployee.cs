    ...
    public partial class AddEmployee : UserControl
    {
        ...
        private void displaymanager()
        {
			if (con.State != ConnectionState.Open)
			{
				...
				{
					con.Open();
					string selectData = "SELECT * FROM emp WHERE (position = 'Project Manager' OR position = 'HR Manager' OR position = 'President') AND delete_date IS NULL AND status = 'Active'";
        ...
        private void AddEmployeeButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Invalid Employee ID. Enter a two-digit number. \n\n" +
                "For Manager  10 + entered two-digit number\nFor Employee 11 + entered two-digit number", "Failed", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ...
            {
                MessageBox.Show("Invalid Phone Number. Number should be 11 digits starting with 03", "Failed", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else if (Regex.IsMatch(AddEmployee_Password.Text, Passpattern) == false)
            {
                errorProvider1.SetError(this.AddEmployee_Password, "Password must be at least 8 characters, must contain at least one Number, one Uppercase Letter, one Lowercase Letter, and one Special Character!");
            }
            else
            {
                // Construct emp_id based on position
                ...
                if (con.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        {
                            checkEm.Parameters.AddWithValue("@EMP_ID", emp_id);
                            int count = (int)checkEm.ExecuteScalar();

                            if (count >= 1)
                            {
                                MessageBox.Show(emp_id + " is already taken", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                    ...
                    try
                    {
                        ...
                        using (SqlCommand checkEm = new SqlCommand(checkEmID, con))
                        {
                            ...
                            else
                            {
                                ...
                                string updateData = "UPDATE emp SET EMP_ID=@EMP_ID , emp_name = @emp_name, contact_num = @contact_num, gender = @gender, " +
                                                    "position = @position, salary = @salary, insert_date = @update_date, status = @status, pass = @pass, " +
                                                    "MGR_ID = @MGR_ID, DEPT_ID = @DEPT_ID WHERE EMP_ID = @EMP_ID";

                                using (SqlCommand cmd = new SqlCommand(updateData, con))
                                {
                                    // Use the constructed emp_id
                                    cmd.Parameters.AddWithValue("@EMP_ID", emp_id);
                                    cmd.Parameters.AddWithValue("@emp_name", AddEmployee_Fullname.Text.Trim());
                                    cmd.Parameters.AddWithValue("@contact_num", AddEmployee_PhoneNumber.Text.Trim());
                                    cmd.Parameters.AddWithValue("@gender", AddEmployee_Gender.Text.Trim());
